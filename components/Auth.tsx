...
const Auth: React.FC<AuthProps> = ({ onLogin }) => {
  ...
  const handleAuthAction = async (provider: 'discord' | 'email') => {
    ...
    try {
      if (provider === 'email') {
        if (!email || !password) {
          throw new Error("Tactical credentials required.");
        }
        ...
        if (isLogin) {
          ...
          if (!acceptTerms) {
            throw new Error("You must accept the tactical protocols.");
          }
      ...
      <div className="w-full max-w-md bg-slate-900/40 backdrop-blur-2xl rounded-[3rem] border border-slate-800/50 p-8 sm:p-10 shadow-2xl relative z-10">
        <div className="flex flex-col items-center mb-8 text-center">
          ...
          <p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.3em] mt-1">
            {isLogin ? 'Tactical Entry Portal' : 'New Strategist Registration'}
          </p>
        </div>
        ...
        <div className="space-y-3">
          ...
          {!isLogin && (
            <div className="flex items-start gap-3 px-1 py-1">
              ...
              <label className="text-[10px] text-slate-500 font-bold cursor-pointer leading-tight select-none pt-0.5" onClick={() => setAcceptTerms(!acceptTerms)}>
                I accept the Battle Protocols and Data Intelligence Privacy Agreement.
              </label>
            </div>
          )}
          ...
          <div className="relative my-6 text-center">
            <span className="text-[9px] uppercase font-black tracking-widest text-slate-600 bg-transparent px-3 relative z-10">Neural Links</span>
            <div className="absolute top-1/2 w-full h-[1px] bg-slate-800/50"></div>
          </div>
          ...
          <div className="grid grid-cols-1">
            <button 
              ...
              disabled={loading}
              className="flex items-center justify-center gap-3 bg-[#5865F2] hover:bg-[#4752C4] py-3.5 rounded-xl transition-all shadow-lg active:scale-95 text-white font-bold text-xs uppercase tracking-widest"
            >
          ...
          <div className="pt-8 text-center">
            <button 
              ...
              className="text-[10px] text-slate-500 hover:text-amber-500 font-black transition-colors underline underline-offset-8 decoration-slate-800 hover:decoration-amber-500/30 uppercase tracking-widest"
            >
